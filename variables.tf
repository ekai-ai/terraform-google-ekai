...
#
# Variables shared across layers with the SAME meaning (project_id, region,
# env, secrets_name, cicd_provider) are declared ONCE here and passed into
# every module block that needs them — see main.tf. They are also simply
# redeclared with the same name/meaning in ./cicd/variables.tf — same tfvars
# file, separate Terraform config.
#
...
# the original codebase — e.g. vpc_name/subnet_name/pods_range_name/
# services_range_name, cluster_name) are NOT declared here. They're wired
# automatically module-to-module in main.tf instead; there is nothing for a
# human to set for them. See the comment block in main.tf and in each
# modules/<layer>/variables.tf for the full list.
#
# cicd_provider default changed from the source codebase's "cloud_build" to
# "none": this repo's examples/self-deploy wrapper and env/customer.tfvars
# template are specifically for self-service client deployments. Unlike the
# AWS distribution's port, the modules/cloud_build and modules/github_actions_cicd
# module directories (and their call sites in modules/cicd/main.tf) are still
# included byte-for-byte here — cicd_provider = "cloud_build" / "github_actions"
# remain genuinely functional if you have the master Secret Manager secret and
# GitHub org those paths expect; "none" is just this repo's shipped default.
# ──────────────────────────────────────────────────────────────────────────────
